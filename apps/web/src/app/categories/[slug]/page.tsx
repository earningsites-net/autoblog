...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="space-y-8 py-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-black/5 bg-white p-4 shadow-card">
        ...
        <div className="flex items-center gap-2 text-sm">
          {pagination.page > 1 ? (
            <Link
              ...
              className="rounded-full border border-black/10 px-4 py-2 hover:border-black/20"
            >
              Previous
            </Link>
          ) : null}
          {pagination.page < pagination.pageCount ? (
            <Link
              href={`/categories/${category.slug}?page=${pagination.page + 1}`}
              className="rounded-full bg-coal px-4 py-2 text-paper hover:bg-coal/90"
            >
              Next
            </Link>
          ) : null}
